...
class Account extends CI_Controller
{
    ...
    public function login()
    {
        ...
        if (!empty($email) && !empty($password)) {
            $data_login = $this->account_model->check_login($email, $password);
            if ($data_login != null) {
                // echo $data_login->id;
                $key_token = $this->account_model->set_token_login($data_login->id);
    ...
    public function register()
    {
        ...
        if (!empty($email) || !empty($password)  || !empty($firstname) || !empty($lastname) || !empty($nickname) || !empty($weight) || !empty($height) || !empty($gender) || !empty($birthday) || !empty($telephone)) {
            $data_regis = $this->account_model->register_user($email, $password, $firstname, $lastname, $nickname, $weight, $height, $gender, $telephone, $birthday, $status, $type);
